...
默认只生成 HTML + PDF，因为 HTML 最容易做到审美在线、可预览，PDF 最适合打印和直接投递。当前 skill 不再生成 Word/DOCX。

PDF 必须优先从 HTML 打印/渲染得到。若本地没有高质量 PDF 生成能力，保留 HTML，并告诉用户可以用浏览器打开后打印为 PDF。不要用低质手工重画 PDF 冒充交付物。

如果用户要求 Word，直接说明本 skill 已取消 Word 交付，只提供 HTML 和 PDF。
...
HTML、PDF 必须从同一个 `resume-data.json` 用同一条命令生成，不要手写其中一种格式后单独交付。推荐：
...
- 三个文件时间接近，避免新旧版本混放。
- `verify_resume_artifacts.py` 通过：HTML 的关键字段、摘要、章节、条目、标签都存在；PDF 至少页数正常、基础身份字段可读，并完成视觉预览。
- 三版均无内部词：`求职说明`、`投递关键词`、`匹配策略`、`匹配：`、`JD 反推`、`ATS`、`缺口分析`、`CONTACT`。
- PDF 渲染为 A4 且中文正常，优先一页，内容丰富时允许两页但不能出现大段空白或断裂。

## 复盘固化原则

这次失败的根因是多格式分裂生成：HTML、Word、PDF 分别走不同临时逻辑，导致文案、视觉和时间戳漂移。通用化后必须遵守：

- Single source of truth：只改 `resume-data.json`，不要直接改某个格式里的正文。
- Single render command：多格式交付用同一次 `render_resume_artifacts.py --formats html,pdf`。
- Single quality gate：交付前运行 `verify_resume_artifacts.py`，并检查 PDF 视觉预览。
- No fake fallback：无法生成高质量 PDF 时只交付 HTML，并说明原因；不要生成或承诺 Word/DOCX。
- No internal vocabulary：`CONTACT`、`匹配策略`、`投递关键词`、`匹配：` 等内部词一律视为交付失败。

推荐在 Codex 桌面环境中使用 workspace bundled Python 运行脚本，因为其中通常包含 `reportlab`、`pypdf`：
